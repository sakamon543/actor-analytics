...
## body_pattern（本文の入り方の配分・**主張型を最優先**）

| body_pattern | 説明 | 配分目安 |
|---|---|---|
| `claim_first` | **ターゲットの問への主張で開く**（「友達からやり直すのが安全は嘘」「外見磨きは必要ない」型）。最強。 | **最低10本** |
| `reader_now` | 読者の今この瞬間の行動・感情を描写して始める（「今、彼のインスタを開いて閉じてを繰り返してるなら」型） | 5本以上 |
| `mechanism_first` | メカニズム解説から始める（事例は補強だけ・自分語りNG） | 5本以上 |
| `reverse_hope` | 反転希望から始める（絶望→希望の急展開・事例なしでもOK） | 3本以上 |
| `mens_voice_quote` | 男の本音セリフから始める（「振った後の彼が〜と言ってた」型・自分の話じゃなく男側のセリフ） | 最大3本 |
| `case_success` | 相談者成功事例から始める | 最大3本 |
| `case_failure` | 失敗警告事例から始める | 最大1本 |

**ルール**：
- 30本中 `claim_first` を最低10本（3分の1）。これが今回の最重要改修
- `case_success` + `case_failure` を**合計4本以下**に厳格制限（前回テンプレ化したから）
...
def build_phase2_prompt(actor, hook, account_info, body_skill_text):
    body_pattern_guide = {
        "claim_first": "ターゲットの問への明確な主張で開く。フック1行目で主張を出した後、続く本文（メイン100-150字）は『その主張の根拠を1行で示唆＋もっと知りたくさせる』流れ。thread で詳細根拠・メカニズム・関連事例を展開。**自分語り（「あの頃の私」「別れて4日目の私」）は冒頭NG・thread の中盤で根拠として使うのは可**。",
    ...
    pattern = hook.get("body_pattern", "claim_first")
