...
def select_scraped_hooks(pool, actor, max_count):
    """この演者が未使用の収集フックを views 降順で max_count 本選ぶ。"""
    avail = [h for h in pool.get("hooks", [])
             if actor not in (h.get("used_by") or {}) and (h.get("first_lines") or "").strip()]
...
def build_phase1_prompt(actor, account_info, target_research, eval_criteria,
    ...
    if scraped_hooks:
        ...
        for s in scraped_hooks:
            ...
            lines.append(f"- pool_id={s.get('pool_id')} [{s.get('views', 0)}views] {fl}")
        scraped_block = f"""
