...
V2_PER_DAY = 9
V2_DAY_PLAN = ["short80", "short80", "short80",
               "mid180", "mid180", "mid180",
               "thread_cta", "thread_cta", "thread_cta"]
...
def build_phase2_prompt(actor, hook, account_info, body_skill_text):
    ...
    if post_type == "short80":
        ...
        task_block = (f"このフックを起点に、**メイン本文(100〜140字)** と **濃いぶら下げ(thread 250〜400字)** の2部構成で必ず書く（読み応え重視の2本立て）。\n"
                      f"【メイン】常識否定 or 男(女)心理の断言を、読者の現在地（「今〜してるあなた」「いま追いLINE消したばかりのあなた」）に重ねて刺す。100字前後の読み応え。\n"
                      f"【ぶら下げ】※この型の主役。次の要素を必ず織り込む：\n"
                      f"  (1) 男・相談者・元彼などの【生のセリフ】を引用（『〜』を複数並べてもよい）。\n"
                      f"  (2) **演者本人の失敗・自爆エピソード** または相談者の具体事例（演者の勝ち筋＝下の商品情報/実績を参照。場面とセリフで描く）。\n"
                      f"  (3) **境界線/タイムリミット**を1つ（「〜を半年放置すると〜に書き換わる」等、動かないリスク）。\n"
                      f"  抽象解説（固定される/印象固定/過去化する 等の名詞化）で埋めず、実際に口で喋る言葉で書く。\n"
                      f"・**threadは絶対に空にしない**（ぶら下げ無し＝不合格）。ぶら下げ末尾のCTAに次のnote直リンクをそのまま貼る：{note_url} （CTA文は中身に合わせ毎回変える／URLは固定）。\n"
                      f"・メイン本文(text)の末尾には「↓」や続き記号を付けない（メインだけでも意味が通る言い切り。ただしぶら下げは必ず別途出力）。")
        output_block = ('{\n  "text": "メイン100〜140字・常識否定/断言＋読者の現在地（末尾に↓なし）",\n'
                        f'  "thread": ["ぶら下げ250〜400字（必須・空禁止）。生セリフ引用＋本人の失敗談or相談事例＋境界線。末尾CTAに {note_url} を貼る"]\n}}')
    else:
...
def run(actor, phase1_only=False, limit=None, reuse_phase1=False):
    ...
    hooks = phase1_result.get("hooks", [])[:len(schedule_slots)]
    # v2演者：日次9本を 短文3/中文3/ぶら下げ付き3 に決定的に振り分ける（slot順＝日跨ぎ順）
    if actor in V2_ACTORS:
        for idx, h in enumerate(hooks):
            h["post_type"] = V2_DAY_PLAN[idx % V2_PER_DAY]
