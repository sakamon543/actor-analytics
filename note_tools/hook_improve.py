...
def build_phase2_prompt(actor, hook, account_info, body_skill_text):
    ...
    if post_type == "short80":
        ...
        task_block = (f"このフックを起点に、**読み応えのあるメイン本文(100〜140字)** と **濃いぶら下げ(thread)150〜200字** の2部構成で必ず書く。"
                      f"薄い1〜2文では終わらせない（短文ポストとは別物。しっかり読ませる2本立て）。\n"
                      f"・メイン：読者の今の状態・行動（今スマホ握って待ってる／毎日即レスしてる等）に触れてから主張を刺す。100字前後の読み応え。\n"
                      f"・ぶら下げ：抽象解説で埋めない。**具体の事例・本人や相談者の実話・男(女)の本音セリフ・数字**で厚くする（演者の勝ち筋＝下の商品情報/実績を参照）。場面とセリフで描く。\n"
                      f"・**threadは絶対に空にしない**（ぶら下げ無し＝不合格）。末尾CTAに次のnote直リンクをそのまま貼る：{note_url} （言い回しは中身に合わせ毎回変える／URLは固定）。\n"
                      f"・メイン本文(text)の末尾には「↓」や続き記号を付けない（メインだけでも意味が通る言い切り。ただしぶら下げは必ず別途出力）。")
        output_block = ('{\n  "text": "メイン100〜140字・読者の現在地＋主張・言い切り（末尾に↓なし）",\n'
                        f'  "thread": ["ぶら下げ150〜200字（必須・空禁止）。具体事例/本音/数字で厚く。末尾CTAに {note_url} を貼る"]\n}}')
    else:
